...
const SidebarHeader = ({ isCollapsed }) => (
  <div className={`flex items-center py-6 border-b border-white/10 transition-all duration-300 ${isCollapsed ? 'px-4 justify-center' : 'px-6 gap-3'}`}>
    ...
    <span className={`text-xl font-bold text-[var(--vanilla)] tracking-tight whitespace-nowrap transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100 ml-3'}`}>Campground</span>
  </div>
);
